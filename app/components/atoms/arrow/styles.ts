import { ArrowAnimation, ArrowStyle, ArrowHeadType } from './types';
...
export const getPathClass = (animation: ArrowAnimation, isSelected: boolean): string => {
  const animClass = animation === 'dash' ? 'animate-dash' : 
                    animation === 'flow' ? 'animate-flow' : '';
...
export const optionsMenuClass = "absolute bg-white rounded-md shadow-lg border border-gray-200 overflow-hidden min-w-[200px] text-xs fade-in z-50 pointer-events-auto";
export const optionButtonClass = "flex items-center justify-between w-full px-3 py-2 text-gray-600 hover:bg-gray-50";
...
export const pathInteractiveStyle = {
  ...
  pointerEvents: 'stroke' as 'stroke',
  zIndex: 30
};
...
export const arrowHeadStyle = (isSelected: boolean) => {
  return {
...
export const getAnimationName = (animation: ArrowAnimation): string => {
  switch (animation) {
    ...
    case 'dash': return 'Línea en movimiento';
    case 'traveling-dot': return 'Punto viajero';
    case 'traveling-dot-fast': return 'Punto viajero rápido';
    case 'traveling-dot-fastest': return 'Punto viajero muy rápido';
...
export const getStyleName = (style: ArrowStyle): string => {
  switch (style) {
    ...
    case 'dashed': return 'Discontinua';
