...
export function CanvasNode({
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    ...
    if (!isDragging && !isResizing) {
      // Solo finalizar conexión si no estábamos arrastrando/redimensionando
    ...
    >
      ...
      {showControls && !isDragging && !isResizing && !isColorPickerOpen && !isIconSelectorOpen && (
        <>
          ...
          <button
            className="absolute -right-6 -top-6 flex h-6 w-6 items-center justify-center rounded-full border border-white bg-red-400 text-white shadow-sm transition-colors hover:bg-red-500"
        ...
        <>
          {[
            ...
            return (
              <div
                key={handle}
                className={`resize-handle absolute h-3 w-3 rounded-full bg-primary ${cursorClass} z-40 border border-white shadow-sm`}
                style={style}
      ...
      {isDragging && (
        <div className="pointer-events-none absolute inset-0 z-0 rounded-md bg-primary/10" />
      )}

      {/* Indicador visual durante el redimensionamiento */}
      {isResizing && (
        <div className="pointer-events-none absolute inset-0 z-0 rounded-md border-2 border-primary" />
      )}
    </div>
  );
}
