...
import { ActiveConnectionType, ConnectionType } from "../types";
...
export function useCanvasConnections({
  ...
  const handleConnectionStart = useCallback(
    (nodeId: string, connectionPosition: ConnectionPosition, x: number, y: number) => {
      ...
      if (activeConnection) {
        ...
        if (getFinalCoordinates) {
          ...
          if (finalCoords.isSnapped && finalCoords.targetNodeId && finalCoords.targetPosition) {
            ...
            logDebug(
              `🧲 Snap aplicado: conectando a ${finalTargetNodeId} (${finalTargetPosition})`
            );
          }
        }

        const newConnection = completeConnection(
          finalTargetNodeId,
          finalTargetPosition,
          finalX,
          finalY
   ...
   */
  const handleUpdateActiveConnection = useCallback(
    (e: React.MouseEvent) => {
      if (activeConnection) {
  ...
  const handleCanvasClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      const isClickOnCanvas =
        ...
        target === document.querySelector("[data-diagram-transform]");
